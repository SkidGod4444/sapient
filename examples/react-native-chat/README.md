...
Dependency security: **`npm audit` is clean** (Expo SDK 54 / RN 0.81). The
seven `tar` advisories that dogged SDK 52 are gone: they had **no fix on the
6.x line** SDK 52's CLI required (overriding to tar 7 breaks `expo prebuild`'s
template extraction — tested), and `@expo/cli` only moved to tar 7 in SDK 53+.
The `overrides` block still pins patched transitive dev-deps
(`@xmldom/xmldom`, `postcss`, `uuid`). Note SDK 53 / RN 0.79 is **not** a
viable stop on the way: its vendored `fmt` fails to compile under Xcode 26
(`consteval` errors) — SDK 54 is the first version that both clears tar and
builds on a current toolchain.
