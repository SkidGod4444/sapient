...
pub fn scaled_dot_product_attention(
    ...
    let vs = v.shape().dims().to_vec();

    if qs.len() != 4 { return Err(SapientError::RankMismatch { expected: 4, got: qs.len() }); }
...
mod tests {
    ...
    fn mha_output_shape() {
        ...
        let q = Tensor::from_f32(&vec![0.1f32; 1 * 2 * 3 * 4], vec![1, 2, 3, 4]).unwrap();
        let k = Tensor::from_f32(&vec![0.1f32; 1 * 2 * 3 * 4], vec![1, 2, 3, 4]).unwrap();
        let v = Tensor::from_f32(&vec![0.1f32; 1 * 2 * 3 * 4], vec![1, 2, 3, 4]).unwrap();
    ...
    fn gqa_kv_repeat() {
        ...
        let q = Tensor::from_f32(&vec![0.1f32; 1 * 4 * 2 * 4], vec![1, 4, 2, 4]).unwrap();
        let k = Tensor::from_f32(&vec![0.1f32; 1 * 2 * 2 * 4], vec![1, 2, 2, 4]).unwrap();
        let v = Tensor::from_f32(&vec![0.1f32; 1 * 2 * 2 * 4], vec![1, 2, 2, 4]).unwrap();
        let out = scaled_dot_product_attention(&q, &k, &v, None, None, 2).unwrap();
    ...
    fn causal_mask_shape() {
        ...
        assert!(d[0 * 3 + 1].is_infinite() && d[0 * 3 + 1] < 0.0);
        // Position (1,0) should be 0
        assert_eq!(d[1 * 3 + 0], 0.0);
    }
}
