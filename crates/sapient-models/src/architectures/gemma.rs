...
pub fn build(info: &ModelInfo) -> Result<Graph> {
    ...
    let mut x = g.add_op(OpType::Mul, vec![embed, scale], 1, Some("embed_scaled".into()));
    ...
    for i in 0..info.num_hidden_layers {
        ...
        let ff_norm = g.add_op(OpType::RmsNorm { epsilon: eps }, vec![x1], 1, Some(format!("{p}.post_feedforward_layernorm")));
        let gate = g.add_op(OpType::MatMul, vec![ff_norm], 1, Some(format!("{p}.mlp.gate_proj")));
        let up   = g.add_op(OpType::MatMul, vec![ff_norm], 1, Some(format!("{p}.mlp.up_proj")));
        let gate_act = g.add_op(OpType::Gelu, vec![gate], 1, Some(format!("{p}.mlp.gelu")));
        let mid  = g.add_op(OpType::Mul, vec![gate_act, up], 1, Some(format!("{p}.mlp.mul")));
        let down = g.add_op(OpType::MatMul, vec![mid], 1, Some(format!("{p}.mlp.down_proj")));
