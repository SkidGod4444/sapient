...
`stats.rs` (`Commands::Stats`, aliases `top`/`monitor`): a ~1 Hz in-place TUI via the `sysinfo` crate — aggregates all `sapient`-named processes' CPU% + RSS, per-core CPU bars (flags the hottest core), system memory, and on-disk footprint (HF model cache via `hub::hub_cache_size()` + binary size). Ctrl-C to exit.
...
- Release is triggered by pushing a `vX.Y.Z` tag; the workflow in `.github/workflows/release.yml` builds all platform binaries including a `-metal` variant for Apple Silicon (MLX) and **`-gpu` variants** for x86_64 Linux (Vulkan) + Windows (DX12), built with `--features wgpu`. **aarch64-linux is cross-compiled from the x86_64 runner with `cargo zigbuild`** (the native `ubuntu-*-arm` runner pool is preemptible and killed release builds mid-link; rustls means no openssl-sys cross blocker — same path as the documented Pi build).
- **Build-variant selection (install + update).** `install.sh`/`install.ps1` auto-detect a GPU on x86_64 (Linux: `/dev/dri/renderD*` or `vulkaninfo`; Windows: `Win32_VideoController`) and download the `-gpu` build when present, else CPU; override with `SAPIENT_VARIANT=cpu|gpu`. `sapient update` (`update.rs`) has `Variant::{Cpu,Metal,Gpu}`; `resolve_variant` offers the accelerated build interactively only when the machine is `metal_capable()` (Apple Silicon) or `gpu_capable()` (x86_64 Linux/Windows with a GPU), or takes `--metal`/`--gpu`/`--cpu`. `current_variant()` reads the compiled feature (`mlx`→Metal, `wgpu`→Gpu, else Cpu).
